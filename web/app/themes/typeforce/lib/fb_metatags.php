...
function build_tags() {
  ...
  if (is_tax('exhibition')) {
    global $wp_query;
    $exhibition_id = $wp_query->queried_object->term_id;
    $metatag_images[] = wp_get_attachment_image_src(get_term_meta($exhibition_id,'_cmb2_featured_image_id',true), 'medium')[0];
  ...
  if(!is_tax('exhibition') && !is_single()){
    ...
    $default_metatag_image = wp_get_attachment_image_src(\Firebelly\SiteOptions\get_option('default_metatag_image_id'), 'medium')[0];
